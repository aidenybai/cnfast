...
import { twMerge as defaultTwMerge } from "./tw-merge.js";
...
export type MergeTemplate = (strings: TemplateStringsArray, values: ClassValue[]) => string;

export const createMergeTemplate = (twMerge: TailwindMerge): MergeTemplate => {
  ...
  return (strings: TemplateStringsArray, values: ClassValue[]): string => {
    const valueCount = values.length;

    // Only cache when every interpolation is a string or falsy: those are immutable, so an identity
    // match guarantees identical output. An object/array interpolation could be mutated between
    // calls while keeping the same reference, so such calls always recompute and are never cached.
    ...
    // Interleave the literal spans with the resolved interpolations exactly as written. Spacing is
    // the author's responsibility (`cn` does not inject spaces between a span and its value),
    // matching how template literals concatenate; the merge engine normalizes the whitespace.
...
export const mergeTemplate = createMergeTemplate(defaultTwMerge);
